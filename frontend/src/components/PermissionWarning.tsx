        ...
        <Alert variant="destructive" className="border-amber-400 bg-amber-50">
          ...
          <AlertTitle className="text-amber-900 font-semibold">
            <div className="flex items-center gap-2">
              ...
              {!hasMicrophone && !hasSystemAudio ? '需要授权' : !hasMicrophone ? '需要麦克风权限' : '需要系统音频权限'}
            </div>
          </AlertTitle>
          {/* Action Buttons */}
          ...
          <AlertDescription className="text-amber-800 mt-2">
            ...
            {!hasMicrophone && (
              <>
                <p className="mb-3">
                  Meetily 需要访问麦克风才能录制会议，但当前未检测到麦克风设备。
                </p>
            ...
            {!hasSystemAudio && (
              <>
                <p className="mb-3">
                  {hasMicrophone
                    ? '系统声音采集不可用。你仍可使用麦克风录音，但不会录到电脑声音。'
                    : '系统声音采集也不可用。'}
                </p>
                {isMacOS && (
                  <div className="space-y-2 text-sm mb-4">
                    <p className="font-medium">在 macOS 上启用系统声音：</p>
                    <ul className="list-disc list-inside ml-2 space-y-1">
                      <li>安装虚拟音频设备（如 BlackHole 2ch）</li>
                      <li>授予 Meetily 屏幕录制权限</li>
